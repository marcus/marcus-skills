#!/usr/bin/env python3
"""Lightroom Catalog Index — searchable SQLite+FTS5 index of Adobe Lightroom CC catalog.

Reads the .mcat SQLite catalog from Lightroom CC, decodes msgpack-encoded
revisions, and writes a clean, searchable SQLite database with full-text search.

Usage:
    lightroom-index.py ingest [--full] [--catalog PATH] [--db PATH]
    lightroom-index.py search <query> [--rating N] [--year YYYY] [--edited] ...
    lightroom-index.py stats
    lightroom-index.py keywords [--top N]
    lightroom-index.py random [--rating N] [--keyword TEXT]

Configuration (in order of precedence):
    CLI flags:     --catalog, --db, --originals
    Env vars:      LIGHTROOM_CATALOG, LIGHTROOM_DB, LIGHTROOM_ORIGINALS
    Auto-detect:   Searches ~/Pictures for *.lrlibrary
"""
...
try:
    ...
    print("Error: msgpack is required. Install it with: pip install msgpack", file=sys.stderr)
    sys.exit(1)
...
def find_catalog():
    """Auto-detect Lightroom CC catalog path on macOS."""
    pictures = os.path.expanduser("~/Pictures")
    pattern = os.path.join(pictures, "*.lrlibrary", "*", "Managed Catalog.mcat")
    matches = glob_mod.glob(pattern)
    if not matches:
        return None
    # Return the most recently modified catalog
    return max(matches, key=os.path.getmtime)


def get_catalog_path(args=None):
    """Resolve catalog path from CLI args, env var, or auto-detection."""
    # CLI flag
    if args and getattr(args, 'catalog', None):
        path = os.path.expanduser(args.catalog)
    ...
    # Env var
    env = os.environ.get('LIGHTROOM_CATALOG')
    if env:
        path = os.path.expanduser(env)
        if not os.path.exists(path):
            print(f"Error: LIGHTROOM_CATALOG path not found: {path}", file=sys.stderr)
            sys.exit(1)
        return path
    ...
    print("Error: Could not find Lightroom catalog.", file=sys.stderr)
    print("  Looked in: ~/Pictures/*.lrlibrary/*/Managed Catalog.mcat", file=sys.stderr)
    print("  Specify manually with --catalog or LIGHTROOM_CATALOG env var.", file=sys.stderr)
    sys.exit(1)


def get_db_path(args=None):
    """Resolve output database path from CLI args, env var, or default."""
    if args and getattr(args, 'db', None):
        return os.path.expanduser(args.db)
    env = os.environ.get('LIGHTROOM_DB')
    if env:
        return os.path.expanduser(env)
    return os.path.join(os.getcwd(), 'lightroom.db')


def get_originals_base(args=None):
    """Resolve originals base path from CLI args, env var, or catalog sibling."""
    if args and getattr(args, 'originals', None):
        return os.path.expanduser(args.originals)
    env = os.environ.get('LIGHTROOM_ORIGINALS')
    if env:
        return os.path.expanduser(env)
    return None
...
def resolve_origin_path(filename, capture_date, originals_base):
    """Resolve file path in originals directory."""
    if not filename or not capture_date or not originals_base:
        return None
    try:
...
def extract_asset(data, doc_id, rev_sequence):
    """Extract asset metadata from msgpack-decoded revision data."""
    ...
    # Rating — structure: {'device_id': {'rating': N, ...}}
...
def cmd_ingest(args):
    catalog_path = get_catalog_path(args)
    ...
    # Get last sequence for incremental
    last_seq = 0
    if not full:
...
def _build_search_query(query, rating=None, rating_exact=None, year=None, edited=False, camera=None, lens=None, date=None, date_from=None, date_to=None):
    conditions = []
    params = []
...
def _format_result(row, with_path=False, originals_base=None):
    r = {
    ...
    if with_path and originals_base:
        path = resolve_origin_path(row[1], row[2], originals_base)
        r['origin_path'] = path
...
def cmd_search(args):
    db_path = get_db_path(args)
    originals_base = get_originals_base(args)
    conn = sqlite3.connect(db_path)
    ...
    results = [_format_result(r, args.with_path, originals_base) for r in rows]
...
def cmd_random(args):
    ...
    result = _format_result(row, with_path=True, originals_base=originals_base)
...
def main():
    parser = argparse.ArgumentParser(
        description='Lightroom Catalog Index — build a searchable SQLite index of your Lightroom CC library',
        epilog="""examples:
  %(prog)s ingest                          # incremental sync (auto-detect catalog)
  %(prog)s ingest --full                   # full rebuild
  %(prog)s ingest --catalog ~/path/to.mcat # specify catalog path
  %(prog)s search "sunset"                 # full-text search
  %(prog)s search "*" --rating 4 --year 2024
  %(prog)s stats                           # library overview
  %(prog)s keywords --top 20              # top keywords
  %(prog)s random --rating 5              # random 5-star photo""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )

    # Global flags
    parser.add_argument('--catalog', help='Path to Lightroom .mcat catalog (default: auto-detect)')
    parser.add_argument('--db', help='Path to output SQLite database (default: ./lightroom.db)')
    parser.add_argument('--originals', help='Path to originals directory (for --with-path resolution)')

    sub = parser.add_subparsers(dest='command')

    p_ingest = sub.add_parser('ingest', help='Index the Lightroom catalog')
    p_ingest.add_argument('--full', action='store_true', help='Full rebuild (default: incremental)')

    p_search = sub.add_parser('search', help='Search indexed photos')
    p_search.add_argument('query', help='Search query (use "*" for all photos)')
    p_search.add_argument('--rating', type=int, help='Minimum star rating')
    p_search.add_argument('--rating-exact', type=int, help='Exact star rating')
    p_search.add_argument('--year', type=int, help='Capture year')
    p_search.add_argument('--edited', action='store_true', help='Only edited photos')
    p_search.add_argument('--camera', help='Camera model/make substring')
    p_search.add_argument('--lens', help='Lens name substring')
    p_search.add_argument('--date', help='Exact capture date (YYYY-MM-DD)')
    p_search.add_argument('--date-from', help='Date range start inclusive (YYYY-MM-DD)')
    p_search.add_argument('--date-to', help='Date range end inclusive (YYYY-MM-DD)')
    p_search.add_argument('--limit', type=int, default=20, help='Max results (default: 20)')
    p_search.add_argument('--with-path', action='store_true', help='Include file paths (requires --originals)')

    p_stats = sub.add_parser('stats', help='Library statistics')

    p_kw = sub.add_parser('keywords', help='Top keywords')
    p_kw.add_argument('--top', type=int, default=50, help='Number of keywords (default: 50)')

    p_rand = sub.add_parser('random', help='Random rated photo')
    p_rand.add_argument('--rating', type=int, help='Minimum star rating')
    p_rand.add_argument('--keyword', help='Keyword filter')
